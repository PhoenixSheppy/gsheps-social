# ARCHIVED IN FAVOR OF UPDATED FORK
# SEE NEW REPO: [gsheps-social-new](https://github.com/PhoenixSheppy/gsheps-social-new)
...
- rewrite shared URLs to reference gsheps.social
...
- by default, gsheps.social (bluesky) should very similar to the official client
...
gsheps.social (bluesky) deployments.
For deer, the intended deployment is with a websever than can serve static files, and reroute to `index.html` as needed. Today [gsheps.social (bluesky)](https://bsky.gsheps.social) is hosted on [cloudflare pages](https://pages.cloudflare.com/).
...
> gsheps.social (bluesky) is a community fork, and we'd love to merge your PR!

As a rule of thumb, the best features for gsheps.social (bluesky) are ones that have a disproportionately positive impact on the user experience compared to the matinance overhead.
Unlike some open source projects, since gsheps.social (bluesky) is a soft fork, any features (patches) we add on top of upstream social-app need to be maintained. For example,
...
Without an overriding motivation, opinionated features should exist behind a toggle that is not enabled by default. This allows gsheps.social to cater to as many users as possible.
...
- Change all branding in the repository and UI to clearly differentiate from gsheps.social (bluesky).
...
Self hosting is great! It is our intention that gsheps.social (bluesky) is easy to self host and build on your own. If you host your own instance of bsky.gsheps.social, or make your own builds, please
...
If the issue pertains to infastructure, code, or systems outside the scope of gsheps.social (bluesky), please refer to the
